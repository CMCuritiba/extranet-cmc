...
```
DEPLOY_ENV=prod
DEPLOY_HOST=extranet.curitiba.pr.leg.br
DEPLOY_PORT=22
DEPLOY_USER=plone
...
prod:
  hosts:
    extranet.curitiba.pr.leg.br:
      ansible_user: root
      host: extranet
      hostname: extranet.curitiba.pr.leg.br
```
...
Deploy the stack defined in `devops/stacks/extranet.curitiba.pr.leg.br.yml` to the remote server with:
